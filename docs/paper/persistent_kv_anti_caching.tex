...
\newcommand{\system}{KVState\xspace}
...
\keywords{KV cache, KV state management, long-context serving, LLM serving, tiered storage}
...
This paper introduces \system, a KV state-management architecture for exact
multi-turn long-context serving. The key idea is to treat historical KV as
serving state that transitions across execution and persistence tiers, rather
than as a best-effort cache block. \system is developed with three designs: 1) a
KV State Index that records committed KV ranges across GPU-ready, CPU-ready,
SSD-cold, fetching, loading, missing, and mismatched states; 2) a KV State Probe,
implemented as KV PrePass, that enumerates required historical KV before a
request enters the LLM engine; and 3) an admission and ready-barrier protocol
that controls when cold KV is allowed to become execution-ready state.
...
This paper borrows the control invariant of database anti-caching for persistent
KV state management. In database anti-caching, main memory is the primary
execution storage, cold tuples are evicted to disk, and an in-memory evicted
table records where each tuple lives~\citep{debrabant2013anticaching}. When a
transaction touches evicted data, the system enters a pre-pass phase to discover
the needed disk blocks, fetches them asynchronously, and restarts the transaction
after the data is available. The same principle fits historical KV reuse with
one crucial change: an LLM engine cannot speculatively execute with missing
historical KV because attention semantics would be incomplete. Therefore, the
state probe must be a control-plane procedure before the request enters the GPU
execution path.

We propose \system, a system design for exact persistent KV state management.
...
\textbf{Second, keep cold-state metadata in memory.} A KV State Index records
...
\textbf{Third, expose cold misses before admission.} A KV State Probe,
implemented as KV PrePass, tokenizes an incoming request, computes a correctness
key, resolves session lineage or prefix candidates, enumerates required
historical ranges, and classifies them as GPU-ready, CPU-ready, SSD-cold,
fetching, loading, missing, or mismatched. The admission rule is explicit:
requests with required cold KV are delayed, requeued, or fallbacked; they are not
allowed to synchronously read SSD in the execution path. This transforms a cold
miss from an opaque TTFT/ITL tail event into a schedulable restore operation.
...
\begin{itemize}[leftmargin=*]
  \item We formulate persistent historical KV serving as a state-management
  problem rather than a conventional cache-hit problem.
  \item We define a multi-tier KV state model and an in-memory KV State Index
  that separates GPU-ready, CPU-ready, SSD-cold, fetching, loading, missing,
  and mismatched states.
  \item We design a KV State Probe, implemented as KV PrePass, and an
  Admission/Ready Barrier protocol that prevents synchronous cold-tier misses
  from entering prefill or decode.
...
\system is inspired by the control invariant in DeBrabant et al.'s anti-caching
architecture for main-memory OLTP databases~\citep{debrabant2013anticaching}.
Anti-caching
...
anti-caching, \system moves the state probe into the serving control plane,
where required KV is resolved from session lineage and prefix manifests before
vLLM execution begins.
...
\system can be viewed as a persistent-state subsystem that could live inside such
a KV-centric architecture. Its distinctive mechanism is the state transition:
...
narrower claim is that multi-turn serving needs persistent state-management
...
including session lineage, prefix manifests, correctness keys, the KV State
...
\subsection{KV State Index}

The KV State Index is the in-memory metadata structure that makes persistent KV
state management possible. It plays the same role as an evicted table in a
database-inspired design: the data itself may be cold, but the system must
always know where it is and whether it can be used.
...
\begin{table*}[t]
  \centering
  \caption{Multi-tier KV states used by the KV State Index.}
  \label{tab:states}
  \begin{tabular}{lll}
    \toprule
...
\subsection{KV State Probe: KV PrePass}

KV PrePass is the state probe for LLM serving. It runs before a request is
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Look up required ranges in the KV State Index.
...
The ready barrier is the state-transition enforcement point for PrePass. It
checks that every required range in the reuse plan is present, verified, and
loadable before decode depends on it. In the prototype, the barrier is
implemented through sidecar admission state and connector metadata. A cold probe
first returns
...
tails. The database analogue is the difference between tuple identifiers and
...
These are engineering gaps in the data path, not changes to the persistent KV
state model.
...
long-lived histories are real. \system adds state-management semantics to this
