...
## 当前阶段
阶段 M3.13 正在推进：项目主线已重新收敛为 **Persistent KV Anti-Caching for Multi-Turn Long-Context Serving**。M3.12 已经证明真实 vLLM + true cold object + PrePass 可以把 cold restore 移出在线请求路径，但 restore 本身仍需要可预测提前量。当前优先级从“证明 PrePass 接口可用”推进为“量化 lead time、restore tail 和 packed cold object 触发点”，先用 M3.13 planner 决定 512/2K/8K/16K/32K 真机矩阵顺序，再进入更大上下文实验。真实 3FS executor 仍属于 P1，不能替代 P0 的 lead-time 与 packed-layout 证据链。
...
| P0 | Packed cold object / extent layout | 每层小文件无法支撑 32K/128K/1M，也无法与 KVDrive/3FS 做严肃对比 |
